...
`service.iptvhelperfixes165.zip`, so each release has its own unique
...
- Repurposes the 2nd/3rd Home widget rows into "New Arabic Movies" and
  "New Arabic TV Shows", pulling directly from specific catalog categories,
  positioned right after Continue Watching. The category IDs are specific
  to this setup's provider, hardcoded into the patch. (An earlier attempt
  inserted brand-new rows into script-skinshortcuts-includes.xml, which
  turned out to be silently regenerated by script.skinshortcuts's own
  build process — any change there gets wiped within minutes. Repurposing
  existing static widget slots avoids that entirely.)
- Reroutes the Bingie skin's main search box to XStream Player's own
  ...
  patch matches four skin files by exact content, not by version number, so
  it safely does nothing to a file instead of risking corruption if a
