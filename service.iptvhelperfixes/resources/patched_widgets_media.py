...
class Media(object):
    ...
    def mylist(self):
        ''' get mylist -- this setup's video library is always empty (pure
        plugin-source content, nothing scanned into Kodi's own library), so
        the original tag-filtered kodidb.movies()/tvshows() query here
        never returned anything. Reads XStream Player's own favorites
        instead, via the same cross-addon Files.GetDirectory technique
        already used elsewhere in this add-on (see get_recently_watched_item
        style callers), falling back to the untouched original library
        query if that add-on isn't installed or returns nothing. '''
        ...
        try:
            ...
            data = _json.loads(files[0].get("label", "{}")) if files else {}
            all_items = data.get("movies", []) + data.get("tvshows", [])
        except Exception as e:
            log_msg("mylist xstream favorites export error: %s" % e)
            all_items = []
        if all_items:
