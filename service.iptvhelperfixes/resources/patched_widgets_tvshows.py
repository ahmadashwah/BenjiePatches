...
class Tvshows(object):
    ...
    def mylist(self):
        ...
        the original tag-filtered kodidb.tvshows() query here never
        returned anything. Reads XStream Player's own favorites instead,
        via the same cross-addon Files.GetDirectory technique used
        elsewhere in this add-on, falling back to the untouched original
        library query if that add-on isn't installed or returns nothing. '''
        ...
        try:
            ...
            tvshows = []
        if tvshows:
            return tvshows[:self.options["limit"]]
        filters = []
        if self.options["hide_watched"]:
            filters.append(kodi_constants.FILTER_UNWATCHED)
